...
class flask_server:
    ...
    def stream_audio_and_transcribe(self):
        """
        STT 처리를 수행하고 결과를 브라우저로 전송.
        """
        process = self.whisper_stt.load_audio_stream(self.stream_url)
        if process is None:
            print("오디오 스트림 로드 실패!")
            return
        ...
        try:
            while True:
                ...
                if len(buffer) >= 16000 * 2:  # 1초 분량
                    ...
                    transcription = self.whisper_stt.transcribe_audio(audio_array)
                    print(f"STT 결과: {transcription}")
                    self.socketio.emit("stt_result", {"text": transcription})  # 결과 전송
    ...
    def start_threads(self):
        ...
        """
        threading.Thread(target=self.generate_hls_stream, daemon=True).start()
        threading.Thread(target=self.stream_audio_and_transcribe, daemon=True).start()
